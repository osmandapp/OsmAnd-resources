//  More realistic, brief and natural commands										 
...
dictionary["distance"] = tts ? ", " : "distance.ogg";
	...
	dictionary["exit"] = tts ? "raus" : "exit.ogg";
...
dictionary["follow2"] = tts ? "weiterfahren" : "follow2.ogg";
	...
	dictionary["around_1_kilometer_nominativ"] = tts ? " Rund einen Kilometer" : "around_1_kilometer_nominativ.ogg";
...
function route_new_calc(dist, timeVal) {
	...
	return dictionary["route_is1"] + " " + distance(dist, "nominativ") + " " + dictionary["route_is2"] + " " + dictionary["time"] + " " + time(timeVal) + (tts ? ". " : "");
}
...
function route_recalc(dist, seconds) {
	return dictionary["route_calculate"] + " " + distance(dist, "nominativ") + " " + dictionary["time"] + " " + time(seconds) + (tts ? ". " : "");
}
...
// roundabout(_Angle, Exit, Street) -- ["take", E, "exit" | Sgen] :- nth(Exit, E), turn_street(Street, Sgen).
	if (dist == -1) {
		...
		return  dictionary["roundabout"] + " " + dictionary["take"] + " " + nth(exit) + " " + dictionary["exit"] + " " + turn_street(streetName);
	}
...
//	return dictionary["prepare"] + getTurnType(turnType) + " " + turn_street(streetName);
}
