...
assemble_street_name(voice([Ref, _, Dest], _), Concat) :- atom_concat(Ref, ' ', C1), atom_concat(C1, Dest, Concat).

turn_street('', []).
turn_street(voice(['','',''],_), []).
turn_street(Street, ['to.ogg', SName]) :- tts, Street = voice(['', '', D], _), assemble_street_name(Street, SName).
...
follow_street(Street, ['to.ogg', SName]) :- tts, Street = voice(['', '', D], _), assemble_street_name(Street, SName).
