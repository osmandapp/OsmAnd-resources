...
function populateDictionary(tts) {
	...
	dictionary["red_light_camera"] = tts ? "radar de semàfor vermell" : "red_light_camera.ogg";
	...
	switch (metricConst) {
		case "km-m":
			if (dist < 17 ) {
				...
				return number_form(Math.round(dist/10.0)*10, "m") + " " + dictionary["meters"];
			} else if (dist < 1000) {
				return number_form(Math.round(2*dist/100.0)*50, "m") + " " + dictionary["meters"];
			} else if (dist < 1500) {
