...
function populateDictionary(tts) {
	// ROUTE CALCULATED
	// TTSエンジン側の仕様変更か文末の最後に『、』がある場合『てん』と読むようになってしまったので駄目な場合は『、』を『。』に変更する
	...
	dictionary["left_sh"] = tts ? "大きく左に曲がってください" : "left_sh.ogg";
	dictionary["left_sl"] = tts ? "左へ進んで下さい" : "left_sl.ogg";
	dictionary["right"] = tts ? "右方向です" : "right.ogg";
	dictionary["right_sh"] = tts ? "大きく右に曲がってください" : "right_sh.ogg";
	...
	//国境検問所あるいは出入国管理施設／出入国管理所、昔は後者だったがイメージし辛かったので変更
	dictionary["border_control"] = tts ? "国境 検問敍があります" : "border_control.ogg"; //国境検問所の所をしょとN2TTSが読むため。半角スペースは国境をこっきょうと読ませるため
	...
	dictionary["toll_booth"] = tts ? "料金所があります" : "toll_booth.ogg";
	dictionary["stop"] = tts ? "一次停止です" : "stop.ogg"; //Googleテキストはこれが一番近いが、N2 TTSはどうにもならない。
	...
	//dictionary["on"] = tts ? "じょうです。" : "on.ogg";    // is used if you turn together with your current street, i.e. street name does not change.
	...
	// とりあえず空白1秒おくことで両立できるようにしていたけれどN2 TTSでは『。』が適度なウェイトになるが、Google読み上げエンジンでは。がウェイトにならない対処法思い浮かばず
	...
	//dictionary["zzz_delay_0500msec"] = tts ? "。。。。。。。。。。" : "zzz_delay_0500msec.ogg";
...
function go_ahead(dist, streetName) {
	if (dist == -1) {
		...
		return distance(dist) + " " + dictionary["follow"] + " " + follow_street(streetName);
	}
}
...
function take_exit(turnType, dist, exitString, exitInt, streetName) {
	...
	return dictionary["in"] + " " + distance(dist) + (tts ? "、 " : " ") + getTurnType(turnType) + " " + getExitNumber(exitString, exitInt) + " " + take_exit_name(streetName);
	// ここまでが追加分
	} else {
		return dictionary["in"] + " " + distance(dist) + (tts ? "、 " : " ") + getTurnType(turnType) + " " + getExitNumber(exitString, exitInt) + " " + take_exit_name(streetName) + " " + dictionary["onto"];
	}
}
...
function take_exit_name(streetName) {
	...
	// 英文版	return (tts ? ", " : " ") + streetName["toStreetName"] + " " + dictionary["toward"] + " " + streetName["toDest"];
...
function getExitNumber(exitString, exitInt) {
	if (!tts && exitInt > 0 && exitInt < 18) {
			...
			//return dictionary["zzz_exit"] + " " + exitString;
...
function roundabout(dist, angle, exit, streetName) {
	...
	if (dist == -1) {
		return nth(exit) + " " + dictionary["exit"] + " " + turn_street(streetName);
	} else {
		...
		return dictionary["in"] + " " + distance(dist) + " " + dictionary["roundabout"] + (tts ? "、 " : " ") + dictionary["and"] + " " + nth(exit) + " " + dictionary["exit"] + " " + turn_street(streetName);
	}
}
