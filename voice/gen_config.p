...
gen(File, fest) :- assert(fest),consult(File), findall(string(Fn, T), string(Fn, T), Result),
		 write_header, fest_language(FL), write('\n(voice_'), write(FL), write(')\n'), write_fsave(Result).
gen(File, google) :- assert(google_gen),!, consult(File), findall(string(Fn, T), string(Fn, T), Result),
