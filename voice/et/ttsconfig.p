...
prepare_turn(Turn, Dist, Street) -- [After, M | Sgen] :- after(Dist) -- After, turn(Turn, M), turn_street(Street, Sgen).
turn(Turn, Dist, Street) -- [After, M | Sgen] :- after(Dist) -- After, turn(Turn, M), turn_street(Street, Sgen).
turn(Turn, Street) -- [M | Sgen] :- turn(Turn, M), turn_street(Street, Sgen).

prepare_make_ut(Dist, Street) -- ['after.ogg', After, 'make_uturn.ogg' | Sgen] :- after(Dist) -- After, turn_street(Street, Sgen).
make_ut(Dist, Street) --  ['in.ogg', After, 'make_uturn.ogg' | Sgen] :- after(Dist) -- After, turn_street(Street, Sgen).
...
distance_mi_f(Dist, Case) -- ['around.ogg', X, Unit]    :- Dist < 16093, !, D is round(Dist/1609.3),            dist(D, Case, X), decline_string('miles.ogg', Case, Unit).
...
string(OggD, A) :- voice_generation, interval(X, 1, 19), atom_number(A, X), atom_concat(A, '.ogg', Ogg), decline_string(Ogg, 'gen', OggD).
string(OggD, A) :- voice_generation, interval(X, 20, 95, 5), atom_number(A, X),  atom_concat(A, '.ogg', Ogg), decline_string(Ogg, 'gen', OggD).
string(OggD, A) :- voice_generation, interval(X, 100, 140, 10), atom_number(A, X), atom_concat(A, '.ogg', Ogg), decline_string(Ogg, 'gen', OggD).
string(OggD, A) :- voice_generation, interval(X, 150, 950, 50), atom_number(A, X), atom_concat(A, '.ogg', Ogg), decline_string(Ogg, 'gen', OggD).
string(OggD, A) :- voice_generation, interval(X, 1000, 9000, 1000), atom_number(A, X),  atom_concat(A, '.ogg', Ogg), decline_string(Ogg, 'gen', OggD).
