...
function populateDictionary(tts) {
	...
	dictionary["enforcement_traffic_signals"] = tts ? "камера праезду на чырвонае святло" : "enforcement_traffic_signals.ogg";
...
function distance(dist) {
	switch (metricConst) {
		...
		case "mi-y":
			if (dist < 17) {
				return (tts ? Math.round(dist/0.9144).toString() : ogg_dist(Math.round(dist/0.9144))) + " " + dictionary["yardau"];
			} else if (dist < 100) {
